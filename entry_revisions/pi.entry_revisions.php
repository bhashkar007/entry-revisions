 ...
 * @link		http://360adaptive.com/entry-revisions-plugin/
 */
...
class Entry_revisions {
	...
	{
		$this->current_site = ee()->config->item('site_id');
		...
		****/
		...
		$channel_id = ee()->TMPL->fetch_param('channel') ? $this->_channel_exists(ee()->TMPL->fetch_param('channel')) : 0; 

		// Parameter # author_id [Optional]
		$author_id = ee()->TMPL->fetch_param('author_id', 0);
		
		// Parameter # entry_id [Optional]
		$entry_id = ee()->TMPL->fetch_param('entry_id', 0);
		
		// Parameter # site_id [Optional]
		$site_id = ee()->TMPL->fetch_param('site_id') ? ee()->TMPL->fetch_param('site_id') : ee()->config->item('site_id');
		...
		$version_date_sort = ee()->TMPL->fetch_param('version_date_sort', 'desc'); 
		...
		if( $entry_id )
			$cond .= " AND exp_entry_versioning.entry_id IN ('".str_replace('|', "','", ee()->db->escape_str($entry_id))."')";

		$cond .= " AND exp_channel_titles.site_id = $site_id";
		$cond .= " AND exp_channel_titles.status IN ('".str_replace('|', "','", ee()->db->escape_str($status))."')";
		
		if( $url_title != '' )
			$cond .= " AND exp_channel_titles.url_title IN  ('".str_replace('|', "','", ee()->db->escape_str($url_title))."')";

		$cond .= ' ORDER BY exp_entry_versioning.version_date '.$version_date_sort;
		$cond .= ' LIMIT '.$limit;
		
		$query = ee()->db->query($sql.$cond);
		...
		if( $query->num_rows() > 0 )
		{
			...
			$this->return_data = ee()->TMPL->parse_variables(ee()->TMPL->tagdata, $tags);
			$query->free_result();
		}
		else
		{
	...
	private function _channel_exists($channel = FALSE)
	{
		...
		$sql = "SELECT GROUP_CONCAT(CAST(channel_id AS CHAR(7)) SEPARATOR ',') AS channel_ids FROM exp_channels WHERE site_id = '{$this->current_site}' AND (channel_id IN ('".str_replace('|', "','", ee()->db->escape_str($channel))."') OR channel_name IN ('".str_replace('|', "','", ee()->db->escape_str($channel))."'))";
...
/* Location: /system/user/addons/entry_revisions/pi.entry_revisions.php */